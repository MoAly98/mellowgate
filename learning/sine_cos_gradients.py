...
def df_relaxed(theta: float) -> float:
    ...
    return (
        dp * (math.sin(theta) - math.cos(theta))
        + p * math.cos(theta)
        - (1.0 - p) * math.sin(theta)
    )
...
def grad_reinforce_mean_baseline(
    theta: float, n_samples: int = 50_000, rng: Optional[RandomGenerator] = None
) -> float:
...
def evaluate_grid(
    thetas: np.ndarray, estimators: Dict[str, Callable[[float], float]]
) -> Dict[str, np.ndarray]:
...
def demo_gradients(
    ...
    estimators = {
        ...
        "Finite Difference MC": lambda th: grad_finite_difference(
            th, n_samples=n_samples, delta=delta, rng=rng
        ),
        "REINFORCE (no baseline)": lambda th: grad_reinforce(
            th, n_samples=n_samples, baseline=None, rng=rng
        ),
        "REINFORCE (mean baseline)": lambda th: grad_reinforce_mean_baseline(
            th, n_samples=n_samples, rng=rng
        ),
        f"Gumbel-Softmax STE (τ={tau})": lambda th: grad_gs_ste(
            th, n_samples=n_samples, tau=tau, rng=rng
        ),
    }
    ...
    for i, (name, vals) in enumerate(results.items()):
        plt.plot(
            thetas, vals, label=name, color=colors[i], linestyle=styles[i], linewidth=2
        )

    plt.xlabel(r"Parameter $\theta$", fontsize=12)
    plt.ylabel(
        r"Gradient estimate $\frac{d}{d\theta}\,\mathbb{E}[f(\theta, k)]$", fontsize=12
    )
    ...
    plt.plot(
        thetas,
        f_relaxed_vals,
        label="Relaxed expectation E[f] (differentiable)",
        linewidth=3,
        color="blue",
    )
    plt.plot(
        thetas,
        f_deterministic,
        label="Deterministic function (non-differentiable at θ=0)",  # type: ignore
        linewidth=3,
        color="red",
        linestyle="--",
    )
...
def plot_decision_probability(savepath: Union[Path, str]) -> Path:
    ...
    plt.plot(
        ...
        p,
        linewidth=2,
        label=r"$p(\theta) = \sigma(\theta) = \frac{1}{1 + e^{-\theta}}$",
    )
...
def plot_stochastic_sampling(
    savepath: Union[Path, str], n: int = 2000, seed: int = 0
) -> Path:
...
def draw_computation_graph(savepath: Union[Path, str]) -> Path:
    ...
    def draw_node(name, text, color="lightblue"):
        ...
        circle = matplotlib.patches.Circle(
            (x, y), 0.04, fill=True, color=color, ec="black"
        )
        ax.add_patch(circle)
    ...
    plt.title(
        "Computation Graph: Stochastic Decision Process f(θ,k)", fontsize=14, pad=20
    )
...
def create_all_visualizations(outdir: Path = Path(".")) -> Dict[str, Path]:
    ...
    paths = {
        "piecewise_function": plot_piecewise_function(
            outdir / "piecewise_function.png"
        ),
        "decision_probability": plot_decision_probability(
            outdir / "decision_probability.png"
        ),
        "stochastic_sampling": plot_stochastic_sampling(
            outdir / "stochastic_sampling.png"
        ),
        "expectation_curves": plot_expectation_curves(
            outdir / "expectation_curves.png"
        ),
...
def animate_stochastic_decision(
    savepath: Union[Path, str], duration: float = 5.0, fps: int = 30
) -> Optional[Path]:
    """
    ...
    anim = animation.FuncAnimation(
        fig, animate, frames=n_frames, interval=1000 / fps, blit=True, repeat=True
    )
...
def create_gradient_flow_animation(
    savepath: Union[Path, str], duration: float = 8.0, fps: int = 20
) -> Optional[Path]:
    """
    ...
    # Animated elements
    (reinforce_line,) = ax1.plot(
        [], [], "r-", linewidth=2, alpha=0.8, label="REINFORCE"
    )
...
def main(config: Optional[DemoConfig] = None):
    ...
    print(
        f"  • Animations: {'Enabled' if config.create_animations and HAS_ANIMATION else 'Disabled'}"
    )
...
def quick_demo(
    n_samples: int = 10_000, create_animations: bool = False
) -> Dict[str, Path]:
    """
...
def compare_estimators_at_point(
    theta: float = 0.5, n_samples: int = 10_000, n_runs: int = 100
) -> None:
    """
    ...
    estimators = {
        "Finite Diff": lambda rng: grad_finite_difference(
            theta, n_samples=n_samples, rng=rng
        ),
        "REINFORCE": lambda rng: grad_reinforce(theta, n_samples=n_samples, rng=rng),
        "REINFORCE+baseline": lambda rng: grad_reinforce_mean_baseline(
            theta, n_samples=n_samples, rng=rng
        ),
    ...
    for name, estimates in results.items():
        ...
        print(
            f"{name:18} | {mean_est:8.4f} | {std_est:8.4f} | {bias:8.4f} | {rmse:8.4f}"
        )
