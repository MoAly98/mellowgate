...
class TestResultsContainer:
    ...
    def test_results_container_creation_minimal(self):
        ...
        assert np.array_equal(container.theta_values, theta_values)
    ...
    def test_results_container_creation_full(self):
        ...
        theta_values = np.array([1.0, 2.0])
        gradient_estimates = {"fd": {"mean": np.array([0.1, 0.2])}}
        sampled_points = {"sampled_indices": np.array([[0, 1], [1, 0]])}
        expectation_values = np.array([1.5, 2.5])
        discrete_distributions = np.array([[0.6, 0.7], [0.4, 0.3]])
        ...
        assert np.array_equal(container.expectation_values, expectation_values)
        assert np.array_equal(container.discrete_distributions, discrete_distributions)

    def test_add_sampled_points_new(self):
        """Test adding sampled points when container has none."""
        container = ResultsContainer(
            theta_values=np.array([1.0]),
            gradient_estimates={"fd": {"mean": np.array([0.1])}},
        )

        sampled_points = np.array([0, 1, 0, 1])
        ...
        assert np.array_equal(container.sampled_points["fd"], sampled_points)

    def test_add_sampled_points_existing(self):
        """Test adding sampled points when container already has some."""
        initial_points = {"reinforce": np.array([1, 0, 1])}
        container = ResultsContainer(
            theta_values=np.array([1.0]),
            gradient_estimates={"fd": {"mean": np.array([0.1])}},
    ...
    def test_add_sampled_points_overwrite(self):
        ...
        assert container.sampled_points is not None
        assert np.array_equal(container.sampled_points["fd"], new_points)
        assert not np.array_equal(container.sampled_points["fd"], original_values)
    ...
    def test_add_expectation_values_overwrite(self):
        ...
        assert container.expectation_values is not None
        assert np.array_equal(container.expectation_values, new_values)
        assert not np.array_equal(container.expectation_values, initial_values)
    ...
    def test_add_discrete_distributions_overwrite(self):
        ...
        new_distributions = np.array([[0.8, 0.9], [0.2, 0.1]])
        container.add_discrete_distributions(new_distributions)

        assert container.discrete_distributions is not None
        assert np.array_equal(container.discrete_distributions, new_distributions)
    ...
    def test_multiple_estimators_gradient_estimates(self):
        ...
        gradient_estimates = {
            ...
            "reinforce": {
                "theta": theta_values,
                "mean": np.array([0.15, 0.25]),
                "std": np.array([0.05, 0.06]),
            },
            "gs": {
                "theta": theta_values,
                "mean": np.array([0.12, 0.22]),
    ...
    def test_empty_arrays(self):
        ...
        gradient_estimates = {
            "fd": {"theta": theta_values, "mean": np.array([]), "std": np.array([])}
        }
